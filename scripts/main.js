...
const translations = {
	en: {
		calendar: {
			...
			"iosInstallHeader": "Install on iOS:",
			"iosStep1": "Open share menu",
			"iosStep2": "Select 'Add to Home Screen'",
			"Select Language": "Select Language",
			"Plate": "Plate",
			"Mechanics": "Mechanics",
			"Health": "Health",
			"storageError": "Storage full! Delete some notes",
	...
	ar: {
		calendar: {
			...
			"ReturnToday": "العودة إلى تاريخ اليوم",
	...
	amz: {
		...
		daysOfWeek: ["Aseggas", "Aynas", "Asinas", "Akras", "Akwas", "Asimwas", "Asiḍyas"]
		},
		cop: {
			calendar: {
				"Calendar": "Ⲑⲟⲣⲙⲓⲁ",
	...
	function showPopup(year, month, day) {
		...
		document.getElementById('saveNote').addEventListener('click', () => {
			...
			try {
				...
				alert(translations[currentLanguage].storageError || 'Storage full! Delete some notes');
			}
			if (noteText) {
  ...
  Math.floor(islamicYear + 622 - (islamicYear / 33)), // Better approximation
  islamicMonth - 1,
  1
);
			generateCalendar(
	...
	function showiOSInstallInstructions() {
		...
		<p>${translations[currentLanguage].iosInstallHeader}</p>
		<ol>
		<li>${translations[currentLanguage].iosStep1}</li>
		<li>${translations[currentLanguage].iosStep2}</li>
		</ol>
		`;
